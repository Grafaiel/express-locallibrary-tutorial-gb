...
exports.index = function(req, res) {
  async.parallel({
        ...
        res.render('index', {title: 'Livraria Local Home', error: err, data: results});
    });
};
...
exports.book_create_post = [
    ...
    body('title', 'Título não pode estar vazio.').trim().isLength( {min: 1} ).escape(),
    body('author', 'Autor não pode estar vazio.').trim().isLength( {min: 1 }).escape(),
    body('summary', 'Sumário não pode estar vazio.').trim().isLength( {min: 1} ).escape(),
    body('genre').escape(),
...
exports.book_delete_get = function(req, res, next) {
    ...
    async.parallel({
        ...
        book_instance: function(callback) {
            ...
            // Successful, so render
            res.render('book_delete', { title: 'Deletar Livro', book: results.book, book_instances: results.book_instance} );
    });
};
...
exports.book_delete_post = function(req, res) {
    ...
    async.parallel({
        ...
        if(results.book_instances.length > 0) {
            // Book has no intances. Render in the same way as GET route.
            res.render('book_delete', {title: 'Deletar Livro', book: results.book, book_instance: results.book_instances} );
            return;
        }
        else {
            // Book has no intances. Delete object and redirect to the list of Books.
...
exports.book_update_get = function(req, res, next) {
    ...
    // Get book, auhtors and genres for form.
    async.parallel({
        book: function (callback) {
        ...
        genres: function (callback) {
            ...
            if (err) { return next(err); }
            if (results.book==null) { //No results.
                let err = new Error('Livro não encontrado');
                err.status = 404;
                return next(err);

            }
            // Success.
            // Mark our selected genres aas checked.
...
exports.book_update_post = [
    ...
    body('author', 'Autor deve ser preenchido.').trim().isLength({ min: 1 }).escape(),
    body('summary', 'Sumário deve ser preenchido.').trim().isLength({ min: 1 }).escape(),
    ...
    (req, res, next) => {
        ...
        if (!errors.isEmpty()) {
            // There are errors. Render form again with sanitized valuer/error messages.
